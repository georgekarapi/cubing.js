import{a as S}from"./chunk-EVMKWAS7.js";import{a as m}from"./chunk-VFJPQATA.js";import{e as x}from"./chunk-S4ZNJEAB.js";import"./chunk-CM3DPZVF.js";import"./chunk-QIOL4UIE.js";import{q as w}from"./chunk-K5RKCHW3.js";import"./chunk-SYRP7SJ5.js";var E=null;async function J(){return E??(E=K())}async function K(){return S(await x.fto.kpuzzle(),`SubgroupSizes 24 12 11 12 12 11 10 9 11 8 10 9 7 8 10 10 6 9 8 5 7 6 5 4 4 8 7 3 6 6 360
...
`)}function p(R){if(R<2)return R;let B=1;for(let L=2;L<=R;L++)B*=L;return B}function d(R,B){if(B<0||B>R)return 0;if(B===0||B===R)return 1;let L=1;for(let F=0;F<B;F++)L=L*(R-F)/(F+1)|0;return L}function Q(R){R=R.slice();let B=R.length,L=p(B-1),F=0;for(;B>1;){B--;let U=R[0];F+=U*L;for(let l=0;l<B;l++){let g=R[l+1];R[l]=g-(g>U)}L/=B}return F}function V(R,B){let L=[],F=p(B-1);for(let U=0;U<B;U++)L[U]=R/F|0,R%=F,F/=B-1-U;for(let U=B-2;U>=0;U--)for(let l=U+1;l<B;l++)L[l]+=+(L[l]>=L[U]);return L}function T(R){let B=R.length,L=0;for(let F=0;F<B-1;F++)for(let U=F;U<B;U++)R[F]>R[U]&&(L^=1);return L}function W(R,B){let L=[],F=p(B-1)/2,U=0;for(let l=0;l<B-1;l++)L[l]=R/F|0,R%=F,F/=B-1-l;L[B-1]=0;for(let l=B-2;l>=0;l--)for(let g=l+1;g<B;g++)L[g]>=L[l]?L[g]++:U^=1;return U===1&&([L[B-2],L[B-1]]=[L[B-1],L[B-2]]),L}function M(R){return Q(R)>>1}function j(R){let B=R.length,L=0;for(let g=0;g<B;g++)L+=+(R[g]===1);let F=B-L;if(F===0||L===0||B===1)return 0;let U=d(B-1,L),l=0;for(let g=0;F>0&&L>0&&B>1;g++)B--,R[g]===0?U=U*--F/B:(l+=U,U=U*L--/B);return l}function X(R,B,L){let F=L-B,U=d(L-1,B),l=[],g=L-1;for(let A=0;A<g;A++)L--,R<U?(l.push(0),U=U*--F/L):(l.push(1),R-=U,U=U*B--/L);return l.push(B),l}function c(R,B){let L=[];for(let F=0;F<B.length;F++)L[F]=R[B[F]];return L}function i(R,B){let L=[];for(let F=0;F<B;F++)L[F]=F;for(let F=0;F<R.length;F++)L[R[F]]=R[(F+1)%R.length];return L}function a(R,B){let L=Array(B).fill(0);for(let F in R)L[F]=R[F];return L}function Y(R,B){let L=Array(8).fill(0);for(let F=0;F<8;F++)L[F]=(R[1][F]+B[1][F])%3;return[c(R[0],B[0]),L]}var Z=[i([0,1,4],12),a({0:2},8)],$=[i([1,2,5],12),a({1:2},8)],BB=[i([2,3,6],12),a({2:2},8)],LB=[i([3,0,7],12),a({3:2},8)],RB=[i([9,8,4],12),a({4:2},8)],FB=[i([10,9,5],12),a({5:2},8)],lB=[i([11,10,6],12),a({6:2},8)],UB=[i([8,11,7],12),a({7:2},8)],bB=[V(0,12),Array(8).fill(0)],s=[Z,$,BB,LB,RB,FB,lB,UB],DB=["UL","U","UR","F","L","B","R","D"],h=[0,1,0,1,1,0,1,0];function gB(R){let B=["0","","'"];return R.map(([F,U])=>DB[F]+B[U]).join(" ")}function AB(){let R=W(Math.floor(m(p(12))/2),12),B=Array(8);for(let L=0;L<8;L++)B[L]=m(3);return[R,B]}function eB(){return tB(AB())}function tB(R){let B=rB(R),L=[z(),q()],F=[oB(),fB()],U=[G(),iB()],l=[aB(),nB()],g=sB(B,L,F),A,D=new Set,r=+new Date;for(let o=0;o<25;o++){let{value:t,done:n}=g.next(),f=R;for(let[u,N]of t)for(let y=0;y<N;y++)f=Y(f,s[u]);if(D.has(f.toString()))continue;D.add(f.toString());let I=M(f[0].slice(0,8)),b=0;for(let u=0;u<4;u++)b+=f[1][u]*3**u;let P=[I,b],H=A?A.length-t.length-1:999999,_=uB(P,U,l,H);if(_!==void 0&&((A===void 0||A.length>t.length+_.length)&&(A=t.concat(_)),new Date-r>300))break}return A}function rB(R){let B=0;for(let l=0;l<4;l++)B+=R[0].indexOf(l+8)*12**l;let L=0;for(let l=0;l<4;l++)L+=R[1][l+4]*3**l;let F=R[0].map(l=>Math.max(-1,l-8)),U=j(F.map(l=>+(l>=0)))*2+T(F.filter(l=>l>=0));return[B,L+81*U]}var e={};function C(){if(e.phase1cm)return e.phase1cm;let R=[];for(let B=0;B<81;B++){R[B]=Array(8);let L=[B%3,Math.floor(B/3)%3,Math.floor(B/9)%3,Math.floor(B/27)];R[B][0]=R[B][1]=R[B][2]=R[B][3]=B;for(let F=0;F<4;F++)L[F]=(L[F]+2)%3,R[B][4+F]=L[0]+L[1]*3+L[2]*9+L[3]*27,L[F]=(L[F]+1)%3}return e.phase1cm=R}function nB(){if(e.phase1cp)return e.phase1cp;let R=Array(81);for(let B=0;B<81;B++){let L=[B%3,Math.floor(B/3)%3,Math.floor(B/9)%3,Math.floor(B/27)];R[B]=(L[0]!==0)+(L[1]!==0)+(L[2]!==0)+(L[3]!==0)}return e.phase1cp=R}function z(){if(e.phase1em)return e.phase1em;let R=[];for(let L=0;L<12;L++){R[L]=[];for(let F=0;F<8;F++)R[L][F]=s[F][0].indexOf(L)}let B=Array(12**4);for(let L=0;L<12**4;L++){B[L]=Array(8);for(let F=0;F<8;F++){let U=0;for(let l=0;l<4;l++)U+=R[Math.floor(L/12**l)%12][F]*12**l;B[L][F]=U}}return e.phase1em=B}function oB(){return e.phase1ep?e.phase1ep:e.phase1ep=v(z(),[8+12*(9+12*(10+12*11))])}function q(){if(e.phase1sm)return e.phase1sm;let R=d(12,4),B=[];for(let U=0;U<R;U++){B[U]=[];let l=X(U,4,12),g=[];for(let A=0,D=0;A<12;A++)l[A]===0?g[A]=-1:g[A]=D++;for(let A=0;A<8;A++){let D=c(g,s[A][0]),r=c(l,s[A][0]),o=T(D.filter(t=>t>=0));B[U][A]=j(r)*2+o}}let L=C(),F=[];for(let U=0;U<R;U++)for(let l=0;l<81;l++){let g=F[l+81*(2*U)]=[],A=F[l+81*(2*U+1)]=[];for(let D=0;D<8;D++)g[D]=L[l][D]+81*B[U][D],A[D]=L[l][D]+81*(B[U][D]^1)}return e.phase1sm=F}function fB(){return e.phase1sp?e.phase1sp:e.phase1sp=v(q(),[0])}function iB(){if(e.phase2cm)return e.phase2cm;let R=C(),B=Array(81);for(let L=0;L<81;L++)B[L]=R[L].slice(4,8);return e.phase2cm=B}function G(){if(e.phase2em)return e.phase2em;let R=8,B=p(R)/2,L=Array(B),F=[0,1,2,3,4,5,6,7];for(let U=0;U<B;U++){L[U]=Array(4);for(let g=0;g<4;g++){let A=c(F,s[g][0].slice(0,8));L[U][g]=M(A)}if(U===B-1)break;let l=0;do for(let g=R-2;g>=0;g--){if(F[g]>F[g+1])continue;let A=g+1;for(let D=A;D<R;D++)F[D]>F[g]&&(A=D);[F[g],F[A]]=[F[A],F[g]],l^=1;for(let D=0;g+1+D<R-1-D;D++,l^=1)[F[g+1+D],F[R-1-D]]=[F[R-1-D],F[g+1+D]];break}while(l!==0)}return e.phase2em=L}function aB(){return e.phase2ep?e.phase2ep:e.phase2ep=v(G(),[0])}function v(R,B){let L=R.length,F=R[0].length,U=Array(L).fill(-1),l=B.slice(),g=[],A=0;for(;l.length>0;){g.length=0;for(let D of l)if(U[D]===-1){U[D]=A;for(let r=0;r<F;r++){let o=R[D][r];for(;o!==D;)g.push(o),o=R[o][r]}}[l,g]=[g,l],A+=1}return U}function uB(R,B,L,F){F=F||999999;let U=R.length,l=0;for(let g=0;g<U;g++)l=Math.max(l,L[g][R[g]]);for(;l<=F;){let g=O(R,B,L,l,-1);if(g!==void 0)return g;l++}}function O(R,B,L,F,U){let l=R.length,g=B[0][0].length,A=0;for(let D=0;D<l;D++)A=Math.max(A,L[D][R[D]]);if(!(A>F)){if(F===0)return[];if(!(A===0&&F===1))for(let D=0;D<g;D++){if(D===U||D<U&&h[D]===h[U])continue;let r=R.slice();for(let t=0;t<l;t++)r[t]=B[t][R[t]][D];let o=1;for(;R.some((t,n)=>R[n]!==r[n]);){let t=O(r,B,L,F-1,D);if(t!==void 0)return[[D,o]].concat(t);for(let n=0;n<l;n++)r[n]=B[n][r[n]][D];o++}}}}function*sB(R,B,L){let F=R.length,U=0;for(let l=0;l<F;l++)U=Math.max(U,L[l][R[l]]);for(;;)yield*k(R,B,L,U,-1),U++}function*k(R,B,L,F,U){let l=R.length,g=B[0][0].length,A=0;for(let D=0;D<l;D++)A=Math.max(A,L[D][R[D]]);if(!(A>F)){if(F===0){yield[];return}if(!(A===0&&F===1))for(let D=0;D<g;D++){if(D===U||D<U&&h[D]===h[U])continue;let r=R.slice();for(let t=0;t<l;t++)r[t]=B[t][R[t]][D];let o=1;for(;R.some((t,n)=>R[n]!==r[n]);){let t=k(r,B,L,F-1,D);for(;;){let{value:n,done:f}=t.next();if(f)break;yield[[D,o]].concat(n)}for(let n=0;n<l;n++)r[n]=B[n][r[n]][D];o++}}}}async function pB(){return new w(gB(eB()))}export{pB as getRandomRediCubeScramble,J as sgsDataFTO};
//# sourceMappingURL=search-dynamic-sgs-unofficial-VSWANF7R.js.map
